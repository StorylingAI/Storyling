...
export const appRouter = router({
  ...
  content: router({
        ...
        (async () => {
          try {
            ...
            try {
              ...
              const thumbnailPrompt = `A photorealistic, cinematic image representing a ${input.theme.toLowerCase()} story titled "${finalTitle}". The scene should be visually stunning, emotionally engaging, and capture the essence of the story. Style: realistic photography, professional lighting, high detail. No text or words in the image.`;
            ...
            let failureReason = "An unexpected error occurred. Please try again.";
            if (rawMessage.includes("quota") || rawMessage.includes("billing") || rawMessage.includes("insufficient_quota")) {
              failureReason = "AI service quota exceeded. Please contact the administrator.";
            } else if (rawMessage.includes("rate_limit") || rawMessage.includes("429")) {
              failureReason = "Too many requests. Please wait a moment and try again.";
            } else if (rawMessage.includes("model_not_found")) {
              failureReason = "AI model configuration error. Please contact the administrator.";
            } else if (rawMessage.includes("ELEVENLABS") || rawMessage.includes("ElevenLabs")) {
              failureReason = "Voice generation service unavailable. Please try again later.";
            } else if (rawMessage.includes("OPENAI_API_KEY") || rawMessage.includes("not configured")) {
              failureReason = "AI service is not configured. Please contact the administrator.";
            } else if (rawMessage.includes("Storage") || rawMessage.includes("storage")) {
              failureReason = "File storage error. Please try again.";
            }
    ...
    regenerateThumbnail: protectedProcedure
      .input(z.object({ 
        contentId: z.number(),
        style: z.enum(["realistic", "illustrated", "minimalist"]).optional()
      }))
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          // Determine style (use input style or existing style or default to realistic)
          const thumbnailStyle = input.style || content.thumbnailStyle || "realistic";
          ...
          await updateGeneratedContent(input.contentId, {
            thumbnailUrl: url,
            thumbnailStyle: thumbnailStyle as "realistic" | "illustrated" | "minimalist",
          });
    ...
    bulkRegenerateThumbnails: protectedProcedure
      .input(z.object({ 
        ...
        style: z.enum(["realistic", "illustrated", "minimalist"])
      }))
      .mutation(async ({ input, ctx }) => {
        const { generateImage } = await import("./_core/imageGeneration");
