...
import { fitSceneTextsToClipCount, splitStoryIntoScenes, generateScenePrompt } from './videoStitching';
...
describe('Video Stitching Module', () => {
  describe('splitStoryIntoScenes', () => {
    ...
    it('should handle short stories gracefully', () => {
      ...
      expect(scenes.length).toBe(6);
      scenes.forEach(scene => {
        expect(scene.replace(/[.!?]/g, '').trim().length).toBeGreaterThan(0);
      });
    });
  ...
  describe('fitSceneTextsToClipCount', () => {
    it('should preserve requested clip count for film-friendly beats', () => {
      const beats = [
        'A traveler arrives at a quiet station.',
        'She walks through a calm market street.',
        'She pauses beside a fountain.',
      ];

      const scenes = fitSceneTextsToClipCount(beats, 6);
