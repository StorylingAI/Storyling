...
export async function generateSubtitleFile(
  ...
  const output = outputPath || path.join('/tmp', `subtitles-${Date.now()}.srt`);

  // Create subtitle entries
  const entries: SubtitleEntry[] = scenes.map((scene, index) => ({
    index: index + 1,
...
export async function addSubtitlesToVideo(
  ...
  const output = outputPath || path.join('/tmp', `video-with-subs-${Date.now()}.mp4`);
  ...
  try {
    ...
    // Using subtitles filter with force_style to apply custom styling
    const command = `ffmpeg -i "${videoPath}" -vf "subtitles='${subtitlePath}':force_style='${styleString}'" -c:a copy -y "${output}"`;
...
export async function generateAndAddSubtitles(
  ...
  const subtitlePath = await generateSubtitleFile(scenes, clipDuration);
