...
export function CookieConsent() {
  ...
  const savePrefs = async (prefs: Record<string, boolean>) => {
    saveLocalPrefs(prefs);  // localStorage first, always
    // Only call the server mutation when logged in — otherwise the global
    // mutation-error handler in main.tsx would redirect to login.
    if (isAuthenticated) {
      try {
        await saveMutation.mutateAsync(prefs as any);
      } catch {
        // Server save failed, localStorage fallback is enough
      }
    }
  };
