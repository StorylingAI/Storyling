...
export function StoryDisplay({
  ...
  const renderWordWithTooltip = (word: string, index: number) => {
    ...
    const cleanWord = word.replace(/[^\w\s\u4e00-\u9fff]/g, "");
  ...
  const renderClickableWords = (text: string) => {
    ...
    const tokens = text.match(/[\w\u4e00-\u9fff]+|[^\w\u4e00-\u9fff]+/g) || [];
    
    return tokens.map((token, index) => {
      // Check if token is a word (not punctuation/whitespace)
      const isWord = /[\w\u4e00-\u9fff]/.test(token);
      ...
      const cleanToken = token.replace(/[^\w\s\u4e00-\u9fff]/g, "");
  ...
  if (lineTranslations && lineTranslations.length > 0) {
    return (
      <div className="space-y-4">
        ...
        <div ref={storyContainerRef} className="space-y-6 overflow-hidden">
  ...
  return (
    <div ref={storyContainerRef} className="prose max-w-none overflow-hidden">
