...
import { Loader2, Upload, ArrowRight, Sparkles, BookOpen, Headphones, Film, Lock } from "lucide-react";
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
                      {PROFICIENCY_LEVELS.map((level) => {
                        ...
                        const isLocked = !isPremium && level !== "A1";
                        return (
                          <Button
                            key={level}
                            ...
                            onClick={() => {
                              if (isLocked) {
                                toast.error(`${level} is available on Premium. Upgrade to unlock all difficulty levels!`);
                                return;
                              }
                              setProficiencyLevel(level);
                            }}
                            className={`rounded-button w-full relative ${
                              proficiencyLevel === level
                                ? "gradient-primary text-white border-0 hover-lift"
                                : isLocked
                          ...
                          >
                            {level}
                            {isLocked && <Lock className="h-3 w-3 ml-1 inline-block" />}
                          </Button>
                        );
                      })}
                    </div>
                    {user?.subscriptionTier !== "premium" && (
                      <p className="text-xs text-muted-foreground mt-1">
                        🔒 A2–C2 levels available with Premium
                      </p>
                    )}
                  </div>
                  ...
                  >
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Watch your story come to life with AI video and built-in narration
                        </p>
                        <div className="text-xs text-muted-foreground mt-2">
                ...
                <div className="space-y-4">
                  ...
                  {mode === "film" && (
                    <>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="film-voice">Story Narration</Label>
                        <Select value={filmNarrationVoiceType} onValueChange={setVoiceType}>
                          <SelectTrigger id="film-voice" className="rounded-xl">
                        ...
                        <p className="text-xs text-muted-foreground">
                          The story audio is embedded in the final video by default.
                        </p>
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="film-gender">Narrator Gender</Label>
                        <Select value={filmNarrationGender} onValueChange={(v) => setNarratorGender(v as "male" | "female")}>
                          <SelectTrigger id="film-gender" className="rounded-xl">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="female">Female</SelectItem>
